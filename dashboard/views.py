...
class AdminDashboardView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        # Users by role
        users_by_role = (
            users.values("user_role").annotate(count=F("id")).order_by("user_role")
        )

        users_by_role_plot = go.Figure()
        users_by_role_plot.add_trace(
            go.Pie(
        ...
        user_registration_over_time = (
            users.annotate(month=ExtractMonth("date_joined"))
            .values("month")
            .annotate(count=F("id"))
            .order_by("month")
        )
        ...
        user_registration_over_time_plot.add_trace(
            go.Bar(
                ...
                marker=dict(
                    color=[item["count"] for item in user_registration_over_time]
                ),
            )
        )
        ...
        vendor_rating_distribution = (
            vendor_rating_distribution.groupby("vendor_rating")
            .size()
            .reset_index(name="count")
        )

        vendor_rating_distribution_plot = go.Figure()
        vendor_rating_distribution_plot.add_trace(
            go.Histogram(
        ...
        vendor_rating_distribution_plot = pio.to_json(vendor_rating_distribution_plot)
...
class ProcurementOfficerDashboardView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        top_items_stock_quantity = inventory_items.order_by("-stock_quantity")[:5]

        top_items_stock_quantity_plot = go.Figure()
        top_items_stock_quantity_plot.add_trace(
            go.Bar(
        ...
        top_items_unit_price_plot.add_trace(
            go.Bar(
                ...
                textposition="outside",
                marker=dict(color=[item.unit_price for item in top_items_unit_price]),
            )
        )
        ...
        top_items_total_price = inventory_items.annotate(
            total_price=F("stock_quantity") * F("unit_price")
        ).order_by("-total_price")[:5]

        top_items_total_price_plot = go.Figure()
        top_items_total_price_plot.add_trace(
            go.Bar(
                x=[item.item_name for item in top_items_total_price],
                y=[
                    item.stock_quantity * item.unit_price
                    for item in top_items_total_price
                ],
                text=[
                    f"${item.stock_quantity * item.unit_price}"
                    for item in top_items_total_price
                ],
                textposition="outside",
                marker=dict(
        ...
        inventory_items_added = (
            inventory_items.annotate(month=ExtractMonth("date_added"))
            .values("month")
        ...
        inventory_items_added_plot.add_trace(
            go.Bar(
                ...
                textposition="outside",
                marker=dict(color=[item["count"] for item in inventory_items_added]),
            )
        )
        ...
        inventory_age["date_added"] = pd.to_datetime(inventory_age["date_added"])
        inventory_age["last_updated"] = pd.to_datetime(inventory_age["last_updated"])
        inventory_age["age"] = (
            inventory_age["last_updated"] - inventory_age["date_added"]
        ).dt.days
        ...
        stock_quantity_distribution = (
            stock_quantity_distribution.groupby("location")
            .agg({"stock_quantity": "sum"})
            .reset_index()
        )

        stock_quantity_distribution_plot = go.Figure()
        stock_quantity_distribution_plot.add_trace(
            go.Pie(
        ...
        stock_quantity_distribution_plot = pio.to_json(stock_quantity_distribution_plot)
        ...
        purchase_requisitions_by_status = (
            purchase_requisitions.values("status")
            .annotate(count=F("id"))
            .order_by("status")
        )

        purchase_requisitions_by_status_plot = go.Figure()
        purchase_requisitions_by_status_plot.add_trace(
            go.Pie(
                labels=[item["status"] for item in purchase_requisitions_by_status],
        ...
        purchase_requisitions_over_time = (
            purchase_requisitions.annotate(month=ExtractMonth("date_created"))
            .values("month")
            .annotate(count=Count("id"))
            .order_by("month")
        )
        ...
        purchase_requisitions_over_time_plot.add_trace(
            go.Bar(
                ...
                textposition="outside",
                marker=dict(
                    color=[item["count"] for item in purchase_requisitions_over_time]
                ),
            )
        )
        ...
        purchase_requisitions_age = pd.DataFrame(
            list(
                purchase_requisitions.values(
        ...
        fig = px.bar(
            ...
            x="inventory__item_name",
            y="age",
            color="quantity_requested",
            title="Purchase Requisitions Age Analysis",
        )
        fig.update_layout(
            xaxis_title="Item Name",
            yaxis_title="Age (Days)",
        ...
        quantity_requested_distribution = pd.DataFrame(quantity_requested_distribution)
        quantity_requested_distribution = (
            quantity_requested_distribution.groupby("inventory__location")
            .agg({"quantity_requested": "sum"})
            .reset_index()
        )

        quantity_requested_distribution_plot = go.Figure()
        quantity_requested_distribution_plot.add_trace(
            go.Pie(
        ...
        # Serializing the data
        serializer = ProcurementOfficerDashboardSerializer(
            data={
...
class VendorDashboardView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        top_bids_bid_amount = supplier_bids.annotate(
            bid_amount=F("unit_price") * F("quantity_fulfilled")
        ).order_by("-bid_amount")[:5]

        top_bids_bid_amount_plot = go.Figure()
        top_bids_bid_amount_plot.add_trace(
            go.Bar(
                ...
                textposition="outside",
                marker=dict(color=[item.bid_amount for item in top_bids_bid_amount]),
            )
        )
        ...
        supplier_bids_by_status = (
            supplier_bids.values("status").annotate(count=F("id")).order_by("status")
        )

        supplier_bids_by_status_plot = go.Figure()
        supplier_bids_by_status_plot.add_trace(
            go.Pie(
                labels=[item["status"] for item in supplier_bids_by_status],
                values=[item["count"] for item in supplier_bids_by_status],
            )
        )
        supplier_bids_by_status_plot.update_layout(title="Supplier Bids by Status")
        supplier_bids_by_status_plot = pio.to_json(supplier_bids_by_status_plot)

        # Supplier Bids over time (monthly)
        supplier_bids_over_time = (
            supplier_bids.annotate(month=ExtractMonth("date_submitted"))
            .values("month")
            .annotate(count=F("id"))
            .order_by("month")
        )
        ...
        supplier_bids_over_time_plot.add_trace(
            go.Bar(
                ...
                textposition="outside",
                marker=dict(color=[item["count"] for item in supplier_bids_over_time]),
            )
        )
        ...
        supplier_bids_over_time_plot = pio.to_json(supplier_bids_over_time_plot)
        ...
        top_orders_order_amount = purchase_orders.annotate(
            order_amount=F("bid__unit_price") * F("bid__quantity_fulfilled")
        ).order_by("-order_amount")[:5]

        top_orders_order_amount_plot = go.Figure()
        top_orders_order_amount_plot.add_trace(
            go.Bar(
        ...
        top_orders_order_amount_plot = pio.to_json(top_orders_order_amount_plot)

        # Purchase Orders by status
        purchase_orders_by_status = (
            purchase_orders.values("status").annotate(count=F("id")).order_by("status")
        )

        purchase_orders_by_status_plot = go.Figure()
        purchase_orders_by_status_plot.add_trace(
            go.Pie(
                labels=[item["status"] for item in purchase_orders_by_status],
                values=[item["count"] for item in purchase_orders_by_status],
            )
        )
        purchase_orders_by_status_plot.update_layout(title="Purchase Orders by Status")
        purchase_orders_by_status_plot = pio.to_json(purchase_orders_by_status_plot)

        # Purchase Orders over time (monthly)
        purchase_orders_over_time = (
            purchase_orders.annotate(month=ExtractMonth("date_ordered"))
            .values("month")
            .annotate(count=F("id"))
            .order_by("month")
        )
        ...
        purchase_orders_over_time_plot.add_trace(
            go.Bar(
                ...
                textposition="outside",
                marker=dict(
                    color=[item["count"] for item in purchase_orders_over_time]
                ),
            )
        )
        ...
        purchase_orders_over_time_plot = pio.to_json(purchase_orders_over_time_plot)
...
def getRoutes(request):
    routes = ["/admin", "/procurement-officer", "/vendor"]
