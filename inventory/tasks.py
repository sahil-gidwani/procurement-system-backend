...
def send_inventory_notifications():
    ...
    procurement_officers = Inventory.objects.values_list('procurement_officer', flat=True).distinct()
    ...
    for officer_id in procurement_officers:
        ...
        item_names = Inventory.objects.filter(procurement_officer=officer_id).values_list('item_name', flat=True)
        ...
        send_mail(subject, message, settings.EMAIL_HOST_USER, [officer_email], fail_silently=True)
