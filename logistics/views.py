...
class InventoryReceiptCreateView(
    ...
    def perform_create(self, serializer):
        ...
        send_logistics_email.delay(instance.id, type='inventory_receipt', action='created')
...
class InventoryReceiptUpdateView(
    ...
    def perform_update(self, serializer):
        ...
        generate_report_and_save.delay(instance.id, 'inventory_receipt')
        send_logistics_email.delay(instance.id, type='inventory_receipt', action='updated')
...
# @method_decorator(cache_page(60 * 15), name="dispatch")
class InventoryReceiptVendorListView(BaseInventoryReceiptVendorAPIView, generics.ListAPIView):
    pass


class InventoryReceiptVendorRetrieveView(BaseInventoryReceiptVendorAPIView, generics.RetrieveAPIView):
    pass
...
class BaseInvoiceProcurementOfficerAPIView(generics.GenericAPIView):
    ...
    def get_queryset(self):
        return Invoice.objects.filter(order__bid__requisition__inventory__procurement_officer=self.request.user)
...
# @method_decorator(cache_page(60 * 15), name="dispatch")
class InvoiceProcurementOfficerListView(BaseInvoiceProcurementOfficerAPIView, generics.ListAPIView):
    pass


class InvoiceProcurementOfficerRetrieveView(BaseInvoiceProcurementOfficerAPIView, generics.RetrieveAPIView):
    pass
...
class InvoicePaymentStatusUpdateView(generics.UpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        generate_report_and_save.delay(instance.id, 'invoice')
        send_logistics_email.delay(instance.id, type='invoice', action='payment_received')
...
class InvoiceVendorRatingUpdateView(generics.UpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        new_rating = (total_ratings * current_rating + serializer.validated_data["vendor_rating"]) / (total_ratings + 1)
