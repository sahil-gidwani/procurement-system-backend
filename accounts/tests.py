...
class ChangePasswordViewTests(APITestCase):
    ...
    def test_change_password_successfully(self):
        ...
        response = self.client.put(self.change_password_url, new_password_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class PasswordResetViewTests(APITestCase):
    ...
    def test_password_reset_with_invalid_email(self):
        ...
        response = self.client.post(self.password_reset_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['email'][0], "User with this email ID doesn't exist.")
...
class PasswordResetConfirmViewTests(APITestCase):
    def setUp(self):
        ...
        self.password_reset_confirm_url = reverse('password_reset_confirm', kwargs={'pk': self.user.pk, 'token': default_token_generator.make_token(self.user)})
    ...
    def test_password_reset_confirm_with_invalid_token(self):
        invalid_token_url = reverse('password_reset_confirm', kwargs={'pk': self.user.pk, 'token': 'invalidtoken'})
        data = {'password': 'newtestpassword', 'confirm_password': 'newtestpassword'}
...
class UserProfileViewTests(SetUpUsers):
    ...
    def test_delete_procurement_officer_profile(self):
        ...
        self.assertRaises(User.DoesNotExist, User.objects.get, username='procurement_officer')
